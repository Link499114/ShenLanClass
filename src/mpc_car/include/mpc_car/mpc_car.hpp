  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (i == 0) {
        ...
        for (int j = i - 1; j >= 0; --j) {
          BB.block(i * n, j * m, n, m) =
              Ad_ * BB.block((i - 1) * n, j * m, n, m);
        }
      ...
      qx.coeffRef(i * n + 0, 0) = -Qx_.coeffRef(i * n + 0, i * n + 0) * xy.x();
      qx.coeffRef(i * n + 1, 0) = -Qx_.coeffRef(i * n + 1, i * n + 1) * xy.y();
